...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="checkuser/userFundCount.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="searchTxt" value="${pd.searchTxt}" placeholder="用户名、姓名检索" />
					...
					<td>
						<input class="btn btn-mini btn-purple" type="button" id="rrr" value="重置"/>
					</td>
				...
				<%-- <tr>
					<td>
						<a href="<%=basePath%>yytraffic/list.do"><button class="btn btn-pink">返回</button></a>
					</td>
				</tr> --%>
				<%-- <tr>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="searchTxt" value="${pd.searchTxt}" placeholder="用户名、项目名检索" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<c:choose>
					<c:when test="${not empty countList}">
						<c:if test="${QX.cha == 1 }">
						<c:forEach items="${countList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										...
										<td>${var.username}</td>
										<td>${var.real_name}
											<c:if test="${var.real_name == null || var.real_name == ''}">-</c:if>
										</td>
										<td>${var.totalRecharge}
											<c:if test="${var.totalRecharge == null || var.totalRecharge == ''}">-</c:if>
										</td>
										<td>${var.totalAccrual}
											<c:if test="${var.totalAccrual == null || var.totalAccrual == ''}">-</c:if>
										</td>
										<td>${var.totalEarnings}
											<c:if test="${var.totalEarnings == null || var.totalEarnings == ''}">-</c:if>
										</td>
										<td>${var.totalDrawcash}
											<c:if test="${var.totalDrawcash == null || var.totalDrawcash == ''}">-</c:if>
										</td>
										<td>${var.floatAmount}</td>
										<td>${var.balance}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>checkuser/excelUserFundList.do?searchTxt='+$('#nav-search-input').val()+'&dateStart='+$('#dateStart').val()+'&dateEnd='+$('#dateEnd').val();
		}
		</script>
