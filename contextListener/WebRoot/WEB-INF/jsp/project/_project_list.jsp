				...
				<tr>
					...
					<td style="vertical-align:top;"> 
					 	<select name="PROJECT_STATE"  id="PROJECT_STATE" data-placeholder="请选择" style="vertical-align:top;width: 120px;">
							...
							<option value="4" <c:if test="${pd.PROJECT_STATE==4}">selected="selected"</c:if>>审核中</option>
					  	</select>
					</td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							...
							<%-- 			<td  class="moneyStyle">${var.BOSS_INTEREST_RATE}%</td> --%>
									...
									<%-- 	<td>${var.TOTAL_PHASE}</td> --%>
										...
										<td id="td_${var.ID }">
										<c:if test="${var.IS_PUBLISH==2 || var.IS_PUBLISH==0 || var.IS_PUBLISH==3}">
											<input type="button" class="btn btn-warning btn-mini" value="未发布"/>
										</c:if>
										<c:if test="${var.IS_PUBLISH==1}">
											<input type="button" class="btn btn-warning btn-mini" value="已发布"/>
										</c:if>
										</td>
										<td id="reject_${var.ID}">
										<c:if test="${var.IS_PUBLISH==0}">
											<input type="button" class="btn btn-warning btn-mini" value="未审核"/>
										</c:if>
