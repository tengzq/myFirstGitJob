...
<body>
	<form action="user/${msg }.do" name="userForm" id="userForm" method="post">
		...
		<table>
			...
			<tr>
				<td>
					<select name="BELONG_TO" id="BELONG_TO" title="所属集团">
						<option value="">请选择所属集团</option>
						<option value="0">无</option>
						<c:forEach items="${groupList}" var="grp">
							<option value="${grp.GROUP_ID}" <c:if test="${pd.BELONG_TO == grp.GROUP_ID}">selected=selected</c:if>>${grp.GROUP_NAME}</option>
						</c:forEach>
					</select>
				</td>
			</tr>
