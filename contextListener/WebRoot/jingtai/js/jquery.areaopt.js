...
//			console.log(bankCode);
			$.ajax({ 
				url: "toQueryBranchInfo.action", 
				async:false,
				type:"POST",
				data:{'region':region,'bankCode':bankCode},
				dataType:"json",
				success: function(data){
					if(data!=null&&data!='null'){
						datasource=eval("("+data+")");
						$("#bankBranchCode").empty();
						$("#bankBranchCode").append("<option value=''><option>");
						for(var i=0;i<datasource.length;i++){
							$("#bankBranchCode").append("<option value='"+datasource[i].code+"'>"+datasource[i].name+"<option>");
						};
					}
				}});
			bankcombobox.prototype.init("#bankBranch");
			var cityCombo = new bankcombobox("#bankBranch", "bankBranchCode");
			cityCombo.dataBind();
			cityCombo.hideFunction = function(){
				$("#bankBranchSpan").css('color', '');
				$("#bankBranchSpan").html('');
			};
		}
