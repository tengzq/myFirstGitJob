function Calendar(elements){
	...
	Calendar.prototype.bindEvent = function() {
		dom.calendar.on('click', selectorClass.p, function(e) {
			...
			var arr=$(".jump-today .year").text().replace('年','-')+$(".jump-today .month").text().replace('月','-')+$(this).attr("data-day");
			$.post("myAccount/dateDaishou",{REPAYMENT_DATE:arr},function(data){
				$(".need-pay .money").find("strong").text(data.pt.ACCRUALALL);
				$(".back-cal-footer").find("strong").text(data.pt.M_ACCRUALALL);
			},"json");
