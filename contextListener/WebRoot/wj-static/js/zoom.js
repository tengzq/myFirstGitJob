$(function() {
	...
	for(var i = 0,len = imgs.length; i < len; i++){
		...
		$(imgs[i]).parent('a').attr('href',src).attr('data-href',src);
	}
    ...
    function openPrevious() {
        var prev = null;//openedImage.parent('div').prev();
    	var imgs = $(openedImage).closest('.gallery').find('img');
    	var index = imgs.index(openedImage.find('img'));
    	if(index > 0){
    		prev = $(imgs[index-1]).parent();
    	}else{
    		prev = $(imgs[imgs.length-1]).parent();
    	}
    	prev.trigger('click');
//    	if (prev.length == 0) {
