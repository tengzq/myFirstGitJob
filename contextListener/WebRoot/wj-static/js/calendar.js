function Calendar(elements){
	...
	function getAccrual(curDate,month){
		...
		$.post("myAccount/dateDaishou",{REPAYMENT_DATE:curDate},function(data){
			if(month){
				//console.log('当前月应收金额：'+data.pt.M_ACCRUALALL);
				$(".back-cal-footer").find("strong").text((data.pt==null || data.pt.M_ACCRUALALL==undefined)?'0.00':data.pt.M_ACCRUALALL);
			}else if(month=='both'){
				$(".back-cal-footer").find("strong").text((data.pt==null || data.pt.M_ACCRUALALL==undefined)?'0.00':data.pt.M_ACCRUALALL);
				$(".need-pay .money").find("strong").text((data.pt==null || data.pt.M_ACCRUALALL==undefined)?'0.00':data.pt.ACCRUALALL);
			}else{
