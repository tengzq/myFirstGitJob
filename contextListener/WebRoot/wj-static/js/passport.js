$(function() {
		...
		$.ajax({
			...
			success :function(r){
				if(r.msg==='success'){
					...
					$('.send-sms').siblings('.err-area').find('.err-tips').text('网络出现问题,稍后再试');
				}
			},
			error : function(){
	...
	setting.elements.form.on('submit', function(e) {
		...
		if ($.inArray('passwordRepeat',setting.custom.check)===-1) {
			...
			if (!ck.ck_username()||!ck.ck_password()||!ck.ck_passwordRepeat()||!ck.ck_phone()||!ck.ck_phoneVerfiyCode()) {
				return false;
			}
		}
		...
		$.post("register/save",{INVITE_ME:$("#INVITE_ME").val(),USERNAME:$.trim(setting.elements.username.val()),PASSWORD:setting.elements.password.val(),TEL:$.trim(setting.elements.phone.val()),CAPTCHA:$.trim(setting.elements.phoneVerfiyCode.val())},function(data){
