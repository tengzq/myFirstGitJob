...
//关闭弹出层
$('.mainlayout .close').click(function(){
	$('.mainlayout').hide();
})
//提交
...
function submitDrawApply(){
	...
	$.ajax({
		...
		success : function(data) {
			if (data.msg == "tologin") {
				window.location.href = basePath+"login_toZfLogin";
			}
			if (data.msg == "faild") {
				swal({
					title : "提现失败",
				...
				function(isConfirm) {
					if (isConfirm) {
						window.location.href = basePath+"myAccount/draw_cash";
					} else {
						window.location.href = basePath+"myAccount/list";
					}
				});
			}
			if (data.msg == "success") {
				window.location.href = basePath+'myAccount/drawcashSuccess';
			}
			if(data.msg == 'captcha'){
				$('#cashSubmit').one('click',function(){
					submitDrawApply();
...
function sendDrawsms(){
	...
	$.ajax({
		...
		success : function(rs){
			if(rs.msg == 'img'){
				$('#btnSmsNum').removeClass('notActive');
