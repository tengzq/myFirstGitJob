...
<!-- saved from url=(0034)https://lantouzi.com/user/recharge -->
...
(function(){
	window.CONFIG = {"PASSPORT_API_HOST":"https:\/\/u.dawanjia.com.cn\/user\/api\/","CAPTCHA_HOST":"https:\/\/u.dawanjia.com.cn\/captcha","ENCRYPT_TIMEOUT":120000,"SERVER_TIME":1432021037000};
...
var _hmt = _hmt || [];
	...
	<script src="../../jingtai/css/PagerView.js"></script>
...
<!-- 			<a href="https://lantouzi.com/zt/festival" target="_blank" style="display:block;position: absolute;right: 0;top: 0;width:170px;height:65px;z-index:100"></a>
	...
	<div class="my-mod my-recharge">
		...
		<div class="bd">
			<p class="available-balance-tips">账户可用余额：<em class="highlight">0.<span>00</span></em>元</p>
							<p style="color:#666">（注：项目起投金额为100的整数倍）</p>
																<form id="rechrage-form" class="form my-form" method="post" action="https://lantouzi.com/user/recharge/pay" target="_blank">
				<input type="hidden" id="now-amount" value="0">
				<ul>
					<li class="form-field">
						<span class="f-k">输入充值金额：</span>
						<span class="f-v">
							<input type="text" name="f[amount]" id="amount" autocomplete="off" style="width:203px;">元
						</span>
						<span class="f-e valid-amount-em" style="clear:left;margin-left:100px;height:42px;">
							<em style="background-position:-380px -298px;"></em>
						</span>
						<span class="f-t" style="margin-left:98px;">充值后账户金额：<em id="ret-amount">0.<span>00</span></em>元</span>
					</li>
				</ul>
				<button type="submit" class="btn tongji" style="margin:20px 100px" data-category="立即充值" data-label="recharge">立即充值</button>
			</form>
			<div class="withdraw-rules">
				<h4>充值说明：</h4>
				<p>1.当日充值金额不可提现，充值未投资金额每日仅可提现10000元。</p>
				<p>2.懒投资严禁信用卡充值、套现等行为，一经发现将予以处罚，包括但不限于：限制收款、 冻结账户、永久停止服务，并会影响银行征信记录。</p>
...
<script type="text/tmpl" id="dialog-tmpl">
	<div class="dg_wrapper dialog-chongzhi" style="height:auto">
		...
		<div class="bd">
			<p>请在新打开的易宝页面完成充值后选择：</p>
			<div class="ret">
				<p>充值成功：<a href="https://lantouzi.com/user/trade/datalist?type=1" class="btn btn-4 tongji" data-category="确认成功" data-label="recharge">确认成功</a></p>
				<p>充值失败：<a href="#" class="btn btn-3 close-btn tongji" data-category="重新充值" data-label="recharge">重新充值</a>&nbsp;&nbsp;<span class="help">查看&nbsp;<a href="https://lantouzi.com/about/help" class="highlight tongji" target="_blank" data-category="查看帮助中心" data-label="recharge">帮助中心</a></span></p>
				<p style="font-size:14px">遇到问题请拨打我们的客服热线：400-807-8000（工作日 9:00-22:00）</p>
			</div>
		</div>
		<a href="javascript:void(0)" class="close tongji" data-category="关闭弹层" data-label="recharge"></a>
	</div>
</script>
...
$(document).ready(function(){
	...
	$amount.on('keyup', function(){
		if(ckAmount()){
			$retMount.text(numberFormat((nowAmount + parseFloat($(this).val() || 0) * 100)/100 ,2));
		}		
	...
	$('#rechrage-form').on('submit', function(e){
		if(!ckAmount()){
			e.preventDefault();
			return;
		}
	...
	<div class="my-nav">
		<ul class="my-nav-1stlevel-list">
			...
			</h3></li><li class="my-nav-1stlevel on"><h3>
	...
	<div class="clearfix ft-bottom">
		...
		<div class="alignright copyright">
			...
			<p><a href="https://lantouzi.com/user/recharge?ua=pc" style="color:#999;padding:0px 5px;" rel="nofollow">PC版</a>
			<a href="https://lantouzi.com/user/recharge?ua=mobile" style="color:#999;padding:0px 5px;text-decoration:underline" rel="nofollow">手机版</a></p>
		</div>
	</div>
	</div>
...
<iframe src="../../jingtais/cm.htm" scrolling="no" width="1" height="1" style="display: none;"></iframe></body></html>