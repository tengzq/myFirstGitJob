...
<!-- saved from url=(0040)https://lantouzi.com/user/order/datalist -->
...
(function(){
	window.CONFIG = {"PASSPORT_API_HOST":"https:\/\/u.dawanjia.com.cn\/user\/api\/","CAPTCHA_HOST":"https:\/\/u.dawanjia.com.cn\/captcha","ENCRYPT_TIMEOUT":120000,"SERVER_TIME":1432020601000};
	...
	<div class="my-mod">
		...
		<div class="bd">
			<div class="my-tabs clearfix">
				<a href="../../jingtai/css/懒投资 - 专为懒人打造的优质理财平台.htm" class="on">全部</a>
				<a href="https://lantouzi.com/user/order/datalist?status=1">募集中</a>
				<a href="https://lantouzi.com/user/order/datalist?status=2&prj_status=4">还款中</a>
				<a href="https://lantouzi.com/user/order/datalist?prj_status=5">已结束</a>
			</div>
					</div>
	</div>
</div>
...
$(function(){
	...
	pager.onclick = function(index){
		...
		ps.prj_status = "";
		if(ps.prj_status.length == 0){
			delete ps.prj_status;
		}
		var url = "https:\/\/lantouzi.com\/user\/order\/datalist" + '?' + $.param(ps);
		location.href = url;
	}
	pager.render();
});
...
$(document).ready(function() {
	var $tips = $('.datalist_tips');
	$tips.on('mouseenter', function(event) {
		event.preventDefault();
		$(this).find('.datalist_tips_content').show();
	}).on('mouseleave', function(event) {
		event.preventDefault();
		$(this).find('.datalist_tips_content').hide();
	});

	var $countPart = $('.countdown');
	$.each($countPart, function(index, val) {
		countDown($(val));
	});

	function countDown($ele){
		var countdown = new Countdown(),
				countdownFmt = Countdown.format,
				$countdown_Time = $ele.find('span'),
				timestring = $ele.data('start'),
				timearr = timestring.split(/[^0-9]/),
				timedata = new Date (timearr[0],timearr[1]-1,timearr[2],timearr[3],timearr[4],timearr[5]),
				endTime = Date.parse(timedata) + 20*60*1000,
				leftTime = endTime - Date.parse(new Date());

		$(countdown).on('countdownProcess', function(e, d){
			$countdown_Time.text(countdownFmt(d).m+'分'+countdownFmt(d).s);
		}).on('countdownFinished', function(){
			$ele.text('您的订单20分钟内未成功支付，系统已自动取消。');
			$ele.siblings('.finishedPay').remove();
		});
		if (leftTime>0) {
	...
	<div class="my-nav">
		<ul class="my-nav-1stlevel-list">
			...
			</h3></li><li class="my-nav-1stlevel">
			<h3><a href="<%=basePath%>jsps/user/draw_card.jsp">提现银行卡</a>
			</h3></li><li class="my-nav-1stlevel"><h3>
			...
			<a href="<%=basePath%>jsps/user/invite_history.jspe">邀请有奖</a>
			</h3></li>		</ul>
	</div>
</div>
			</div>			
	...
	<div class="clearfix ft-bottom">
		...
		<div class="alignright copyright">
			...
			<p><a href="https://lantouzi.com/user/order/datalist?ua=pc" style="color:#999;padding:0px 5px;" rel="nofollow">PC版</a>
			<a href="https://lantouzi.com/user/order/datalist?ua=mobile" style="color:#999;padding:0px 5px;text-decoration:underline" rel="nofollow">手机版</a></p>
		</div>
	</div>
	</div>
