...
public class Page {
	...
	private String portalPageStr;		//最终页面显示的底部翻页导航，详细见：getPageStr();
	private PageData pd = new PageData();
	
	public void setPortalPageStr(String portalPageStr) {
		this.portalPageStr = portalPageStr;
	}
	 ...
	 * 前台分页
	 */
	public String getPortalPageStr() {
		StringBuffer sb = new StringBuffer();
		if(totalResult>0){
			if(currentPage==1){
				sb.append("	<span>|&lt;</span>");
				...
				sb.append("	<a style=\"cursor:pointer;\" onclick=\"nextPage(1)\"><span>|&lt;</span></a>");
				sb.append("	<a style=\"cursor:pointer;\" onclick=\"nextPage("+(currentPage-1)+")\"><span>&lt;&lt;</span></a>");
			}
			int showTag = 5;//分页标签显示数量
			int startTag = 1;
			if(currentPage>showTag){
			...
			for(int i=startTag; i<=totalPage && i<=endTag; i++){
				if(currentPage==i)
					sb.append("<a  ><span class=\"on\">"+i+"</span></a>");
				else
					sb.append("	<a style=\"cursor:pointer;\" onclick=\"nextPage("+i+")\"><span>"+i+"</span></a>");
			}
			if(currentPage==totalPage){
				sb.append("	<span>&gt;&gt;</span>");
				sb.append("	<span>&gt;|</span>");
				...
				sb.append("	<a style=\"cursor:pointer;\" onclick=\"nextPage("+(currentPage+1)+")\"><span>&gt;&gt;</span></a>");
				sb.append("	<a style=\"cursor:pointer;\" onclick=\"nextPage("+totalPage+")\"><span>&gt;|</span></a>");
			}
			sb.append("	<p>一共"+totalPage+"页, "+totalResult+"条记录</p>\n");
			sb.append("<script type=\"text/javascript\">\n");
			//换页函数
			sb.append("function nextPage(page){");
			sb.append("	if(true && document.forms[0]){\n");
		...
		portalPageStr = sb.toString();
		return portalPageStr;
	}
